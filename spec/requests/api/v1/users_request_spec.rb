...
RSpec.describe "Api::V1::Users", type: :request do
  ...
  describe 'updates user' do
    context 'with valid attributes' do
      it 'updates user' do
        patch api_v1_user_url(user), params: { user: { email: user.email, password: 'Test1234#'} }, as: :json
        expect(response).to have_http_status(:success)
      end
    end
    context 'with invalid attributes' do
      it 'does not update the user' do
        patch api_v1_user_url(user), params: { user: { email: 'badmail', password: 'Test1234#'} }, as: :json
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
  ...
  describe 'deletes user' do
    it 'deletes user' do
      ...
      expect do
        delete api_v1_user_url(user_to_delete), as: :json
